...
**Resolution (this branch).** Four of the five findings are fixed in the commits that accompany this report: electric-thrust derating (`advanceThrustShip` now evaluates `thrustAt(stage, r_helio)` per segment, a no-op for chemical stages, with a regression test at 3 AU); Jupiter J2 updated to the Juno value `0.0146965`; the `sim.ts` determinism comment scoped to match `serialize.ts`; and a hyperbolic coe2rv round-trip test added to `kepler.test.ts`. The **comms relativistic-convergence** finding is intentionally left as a recommendation — the real fix is an algorithmic change (accelerated/closed-form solve) to a core file with a document-vs-rewrite design choice, deferred pending an explicit decision.
